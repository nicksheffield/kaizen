...
const tmpl = ({ project, extras }: { project: ProjectCtx; extras: HonoGeneratorExtras }) => {
	...
	export const sendVerificationEmail = async (
		...
		await send({
			...
			`
					: `body: \`Your verification code is: \${code}. Enter it <a href="http://localhost:3001/confirm-account?userId=\${userId}">here</a>.\``
			}
		})
	}
	...
	export const sendPasswordResetToken = async (email: string, code: string) => {
		await send({
			...
			`
					: `body: \`Enter your new password <a href="http://localhost:3001/reset-password?code=\${code}">here</a>.\``
			}
		})
	}
